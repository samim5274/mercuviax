<template>
    ...
    >
        <div class="mx-auto max-w-7xl px-4 py-12 sm:px-6 sm:py-16 lg:px-10 lg:py-24">
            <!-- Section Header -->
            <div class="mb-10 flex flex-col items-center text-center sm:mb-12">
                ...
                <h2 class="text-2xl font-black tracking-tight text-slate-900 sm:text-4xl lg:text-5xl dark:text-white">
                    Transparent Pricing Plans
                </h2>

                <p class="mt-4 max-w-2xl text-sm leading-relaxed text-slate-600 sm:text-base dark:text-slate-400">
            ...
            <div class="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-xs dark:border-white/5 dark:bg-slate-900/40">
                <!-- Desktop / large-tablet table (lg and up: 7 columns need the room) -->
                    ...
                    <table class="w-full text-left text-sm border-collapse">
                        <thead class="border-b-2 border-teal-600/20 bg-gradient-to-r from-slate-50 to-slate-100/50 text-xs font-semibold uppercase tracking-wider text-slate-700 dark:border-teal-400/10 dark:from-slate-900 dark:to-slate-900/50 dark:text-slate-300">
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-4 text-right">Monthly</th>
                        ...
                        <tbody class="divide-y divide-slate-200/70 dark:divide-white/5">
                            ...
                            >
                                <td class="px-6 py-4 font-semibold tracking-tight text-slate-900 dark:text-white">
                                    {{ item.type }}
                                </td>
                                ...
                                <td class="whitespace-nowrap px-6 py-4 text-right">
                                    <span
                                        v-if="item.monthlyPayment"
                                        class="inline-flex items-center gap-1.5 rounded-full bg-teal-50 px-2.5 py-1 text-xs font-semibold text-teal-700 dark:bg-teal-400/10 dark:text-teal-400"
                                    >
                                        {{ item.monthlyPayment }}
                                    </span>
                                    <span v-else class="text-xs font-medium text-slate-400 dark:text-slate-600">
                                        &mdash;
                                    </span>
                                </td>
                    ...
                    >
                        <div>
                            <div class="flex items-start justify-between gap-2 border-b border-slate-200/60 pb-3 dark:border-white/5">
                                <h3 class="font-bold leading-snug text-slate-900 dark:text-white">
                                    {{ item.type }}
                                </h3>
                                <span class="shrink-0 rounded-md bg-amber-500/10 px-2 py-0.5 text-xs font-semibold text-amber-600 dark:text-amber-400">
                                    Offer
                                </span>
                            </div>
                            ...
                            <div class="mt-4 space-y-2.5 text-xs">
                                <div class="flex items-start justify-between gap-3">
                                    <span class="shrink-0 text-slate-500 dark:text-slate-400">Framework:</span>
                                    <span class="text-right font-medium text-slate-700 dark:text-slate-300">{{ item.framework }}</span>
                                </div>
                                <div class="flex items-start justify-between gap-3">
                                    <span class="shrink-0 text-slate-500 dark:text-slate-400">Domain & Hosting:</span>
                                    <span class="text-right font-medium text-slate-600 dark:text-slate-400">{{ item.domainHosting }}</span>
                                </div>
                                <div class="flex items-start justify-between gap-3">
                                    <span class="shrink-0 text-slate-500 dark:text-slate-400">Logo & Banner:</span>
                                    <span class="text-right font-semibold text-teal-600 dark:text-teal-400">✓ {{ item.logoBanner }}</span>
                                </div>
                                <div class="flex items-start justify-between gap-3">
                                    <span class="shrink-0 text-slate-500 dark:text-slate-400">Monthly:</span>
                                    <span
                                        class="text-right font-semibold"
                                        :class="item.monthlyPayment ? 'text-teal-600 dark:text-teal-400' : 'text-slate-400 dark:text-slate-600'"
                                    >
                                        {{ item.monthlyPayment || '—' }}
                                    </span>
                                </div>
